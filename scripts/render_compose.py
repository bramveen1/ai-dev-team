...
def _router_service(agent_names: list[str]) -> dict:
    ...
    # Shared bearer token for the router's compose-internal HTTP API on port 8090.
    # Injected into the router and every agent container so the dispatch.draft verb
    # can POST to http://router:8090/internal/drafts without exposing the port to
    # the host. Rotation = update data/secrets.json + redeploy.
