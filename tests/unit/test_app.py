...
# ── _resolve_workers_bot_user_id ChatAdapter routing (#842, #863) ────
#
...
# path. #863 retired the module's rollout flag (APP_LIFECYCLE_VIA_CHAT_ADAPTER
# in router/settings.py is now default-on and unconditional; this module no
# longer reads it), so the matrix below is no longer parameterized by flag
# state:
# - No/Slack transport (today's one call site) → identical
#   AsyncWebClient(token=...) + auth.test call, always.
# - Non-Slack transport → log-and-skip, AsyncWebClient never built — there is
#   no ChatAdapter equivalent of "the workers bot's Slack user id".


class TestResolveWorkersBotUserIdChatAdapterRouting:
    @pytest.mark.asyncio
    async def test_no_transport_uses_legacy_path(self, app_module, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_slack_transport_uses_legacy_path(self, app_module, monkeypatch):
        monkeypatch.setenv("WORKERS_BOT_TOKEN", "xoxb-workers-842")
    ...
    @pytest.mark.asyncio
    async def test_non_slack_transport_skips_without_slack_fallback(self, app_module, monkeypatch, caplog):
        monkeypatch.setenv("WORKERS_BOT_TOKEN", "xoxb-workers-842")
    ...
    @pytest.mark.asyncio
    async def test_unsupported_transport_skips_without_slack_fallback(self, app_module, monkeypatch):
        monkeypatch.setenv("WORKERS_BOT_TOKEN", "xoxb-workers-842")
