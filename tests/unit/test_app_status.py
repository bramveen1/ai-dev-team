...
Uses deferred imports to avoid triggering AsyncApp() initialization at
module level (which requires SLACK_BOT_TOKEN).
"""
...
class TestSetAssistantStatus:
    ...
    @pytest.mark.asyncio
    async def test_calls_assistant_threads_set_status(self, app_module, mock_slack_client):
        await app_module.set_assistant_status(mock_slack_client, "C0001", "1.0", "is thinking\u2026")
    ...
    @pytest.mark.asyncio
    async def test_swallows_errors(self, app_module, mock_slack_client):
...
class TestHandleEventStatus:
    ...
    async def test_sets_assistant_status_before_dispatch(
        self, app_module, mock_slack_client, mock_dispatch, mock_session, mock_exit_trigger
    ):
    ...
    @pytest.mark.asyncio
    async def test_posts_response_via_say(
        self, app_module, mock_slack_client, mock_dispatch, mock_session, mock_exit_trigger
    ):
    ...
    @pytest.mark.asyncio
    async def test_no_reactions_add_called(
        self, app_module, mock_slack_client, mock_dispatch, mock_session, mock_exit_trigger
    ):
...
class TestHandleEventStatusErrors:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_error_posts_error_via_say(
        self, app_module, mock_slack_client, mock_session, mock_exit_trigger
    ):
...
class TestDefaultThinkingStatus:
    ...
    def test_default_thinking_status_exists(self, app_module):
        """DEFAULT_THINKING_STATUS should be defined as a fallback."""
        assert app_module.DEFAULT_THINKING_STATUS == "is thinking\u2026"
