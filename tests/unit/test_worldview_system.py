"""Unit tests for the WORLDVIEW system — shared personality + per-agent role separation.

Tests verify:
- WORLDVIEW.md is loaded and included in context
- personality.md is loaded per-agent and included in context
- Loading order: WORLDVIEW -> role -> personality -> memory -> org memory
- No duplication between WORLDVIEW and personality content
...
class TestWorldviewInContext:
    """Tests that WORLDVIEW.md content is included in assembled context."""

    def test_context_includes_worldview(self):
        """build_context() should include WORLDVIEW content when provided."""
        result = build_context(
            role_md="# Lisa\nProject manager.",
            memory="",
            thread_history=[],
            system_docs="",
            worldview_md="# WORLDVIEW\nBe genuinely helpful.",
        )
    ...
    def test_context_includes_both_worldview_and_personality(self):
        """build_context() should include both WORLDVIEW and personality."""
        result = build_context(
            role_md="# Lisa\nProject manager.",
            memory="",
            thread_history=[],
    ...
    def test_worldview_before_role(self):
        """WORLDVIEW should appear before role in assembled context."""
        result = build_context(
            role_md="ROLE_MARKER",
            memory="",
            thread_history=[],
            system_docs="",
            worldview_md="WORLDVIEW_MARKER",
        )
        assert result.index("WORLDVIEW_MARKER") < result.index("ROLE_MARKER")
    ...
    def test_full_loading_order(self):
        """Full context should follow order: WORLDVIEW -> role -> personality -> memory -> docs -> thread."""
        result = build_context(
            role_md="ROLE_MARKER",
            memory="MEMORY_MARKER",
        ...
        positions = {
            "worldview": result.index("WORLDVIEW_MARKER"),
        ...
        assert positions["worldview"] < positions["role"]
...
class TestWorldviewWithEmptySections:
    """Tests for graceful handling of missing WORLDVIEW/personality content."""

    def test_empty_worldview_omitted(self):
        """Empty worldview_md should not add blank sections to context."""
        result = build_context(
            role_md="# Lisa",
            memory="",
            thread_history=[],
    ...
    def test_whitespace_only_worldview_omitted(self):
        """Whitespace-only worldview_md should be treated as empty."""
        result = build_context(
            role_md="# Lisa",
            memory="",
            thread_history=[],
    ...
    def test_backwards_compatible_without_worldview_personality(self):
        """Calling build_context without worldview/personality should work as before."""
        result = build_context(
            role_md="# Lisa\nSenior developer.",
            memory="## Recent\nFixed auth.",
...
class TestLoadMemoryFunction:
    ...
    def test_load_worldview_file(self, test_memory_dir):
        """Should load WORLDVIEW.md from the shared directory."""
        result = load_memory(test_memory_dir / "shared" / "WORLDVIEW.md")
        assert "genuinely helpful" in result
    ...
    def test_load_missing_file(self):
        ...
        result = load_memory("/tmp/nonexistent_worldview_12345.md")
        assert result == ""
...
class TestLoadAgentContext:
    ...
    def test_load_agent_context_includes_worldview(self, test_memory_dir):
        """Agent context should include WORLDVIEW content."""
        result = load_agent_context(
            agent_name="lisa",
            shared_dir=test_memory_dir / "shared",
            agent_dir=test_memory_dir / "agents" / "lisa",
    ...
    def test_load_agent_context_order(self, test_memory_dir):
        """Context should be loaded in order: worldview, role, personality, agent_memory, org_memory."""
        result = load_agent_context(
            agent_name="lisa",
            shared_dir=test_memory_dir / "shared",
            agent_dir=test_memory_dir / "agents" / "lisa",
        ...
        if "worldview" in labels and "personality" in labels:
            assert labels.index("worldview") < labels.index("personality")
    ...
    def test_no_duplication_between_worldview_and_personality(self, test_memory_dir):
        """WORLDVIEW content and personality content should not overlap."""
        result = load_agent_context(
            agent_name="lisa",
            shared_dir=test_memory_dir / "shared",
            agent_dir=test_memory_dir / "agents" / "lisa",
        ...
        if "worldview" in contents and "personality" in contents:
            # Personality should not repeat WORLDVIEW's core rules
            worldview_lines = set(contents["worldview"].strip().splitlines())
            personality_lines = set(contents["personality"].strip().splitlines())
            # Headers can overlap (e.g. "# "), but content lines should not
            content_overlap = worldview_lines & personality_lines - {"", "#"}
