...
The Browser Use wiring (actually driving Chromium) is **stubbed** in
this PR — each action returns ``{"status": "not_implemented"}`` so the
sidecar can be exercised end-to-end (start → /health → /api → exit
cleanly) without pulling in the full Chromium dependency in unit
tests. Wiring the Browser Use ``Agent`` into ``_dispatch_action`` is
the immediate follow-up.

The Browser Use library + Playwright Chromium are still installed in
the sidecar image (see ``docker/Dockerfile.browser``) so the follow-up
is just module work, not infra work.
"""
...
def _dispatch_action(action: str, profile: Profile, bundle: SecretBundle, payload: dict[str, Any]) -> dict[str, Any]:
    """Run one browser action.

    Browser Use wiring is stubbed in this PR — each action returns a
    structured ``not_implemented`` response. The follow-up replaces
    this dispatcher with a real ``browser_use.Agent`` invocation that
    consumes ``bundle.values`` as env vars and writes session state
    into ``profile.path``.
    """
    if action == "navigate":
        url = payload.get("url")
        ...
        logger.info("navigate profile=%s url=%s (stub)", profile.name, url)
        return {"status": "not_implemented", "action": action, "profile": profile.name, "url": url}
    if action == "extract":
        selector_map = payload.get("selectors") or {}
        if not isinstance(selector_map, dict):
            raise HTTPException(status_code=400, detail="extract 'selectors' must be an object")
        logger.info("extract profile=%s selectors=%d (stub)", profile.name, len(selector_map))
        return {"status": "not_implemented", "action": action, "profile": profile.name}
    if action == "screenshot":
        logger.info("screenshot profile=%s (stub)", profile.name)
        return {"status": "not_implemented", "action": action, "profile": profile.name}
...
def _build_app() -> FastAPI:
    ...
    @app.post("/api/{action}")
    def run_action(action: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        result = _dispatch_action(action, profile, bundle, body)
        # Scrub any echoed plaintext on the way out. This catches a
        # buggy action handler that accidentally returns env values in
        # its response body. It cannot defeat semantic leaks (the secret
        # encoded differently or split across keys); the caller and the
        # action handlers must avoid those at the source. README's
        # "Log scrubbing" section spells this out.
        scrubbed = {k: bundle.scrub(str(v)) if isinstance(v, str) else v for k, v in result.items()}
        return scrubbed
