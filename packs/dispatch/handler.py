...
from slots import release_slot as _release_slot  # noqa: F401 — re-exported for tests/cancel paths
from slots import release_slot_for_dispatch as _release_slot_for_dispatch
...
# (POOL_SIZE and POOL_QUEUE_DIR_NAME live in slots.py; re-exported above.)
...
# Implementation moved to pr_review.py (wave 3b); re-exported here so the CLI
# surface and direct test calls keep working. _pr_review_settings stays as a
# thin wrapper below, forwarding this module's quota globals so existing
# test patch targets on handler keep working.
from pr_review import (  # noqa: E402, F401 — back-compat re-exports
    _PR_REVIEW_EVENT_MAP,
    _PR_REVIEW_UNCONFIGURED_DETAIL,
    PR_REVIEW_IDENTITY_ENV,
    PR_REVIEW_TOKEN_PATH_ENV,
    PR_REVIEW_VERDICTS,
    SAM_REVIEW_TOKEN_PATH_ENV,
    _check_pr_identity_conflict,
    _parse_pr_url_for_review,
    _read_review_token,
    _verify_review_identity,
    pr_review,
    pr_review_health,
)
...
def _pr_review_settings() -> "dict[str, Any]":
    ...
    Thin wrapper over :func:`pr_review.resolve_settings`, forwarding this
    module's quota globals so tests that patch ``handler._QUOTA_CONFIG_PATH``
    / ``handler._quota`` keep steering the resolution.
    """
    import pr_review as _pr_review_mod  # noqa: PLC0415 — sibling module, cheap

    return _pr_review_mod.resolve_settings(
        quota_mod=_quota if (_QUOTA_AVAILABLE and _quota is not None) else None,
