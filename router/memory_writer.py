...
async def persist_memory(
    ...
    try:
        for decision in memory_updates.get("decisions", []):
            if not isinstance(decision, dict):
                logger.warning("Skipping non-dict decisions item: %r", decision)
                continue
        ...
        logger.exception("Error persisting decisions for agent=%s; skipping field", agent_name)

    # Preferences → config/agents/{agent}/memory/preferences/preferences.md
    try:
        for pref in memory_updates.get("preferences", []):
            if not isinstance(pref, dict):
                logger.warning("Skipping non-dict preferences item: %r", pref)
                continue
        ...
        logger.exception("Error persisting preferences for agent=%s; skipping field", agent_name)

    # People → config/agents/{agent}/memory/people/{name}.md
    try:
        for person in memory_updates.get("people", []):
            if not isinstance(person, dict):
                logger.warning("Skipping non-dict people item: %r", person)
                continue
        ...
        logger.exception("Error persisting people for agent=%s; skipping field", agent_name)

    # Projects → config/agents/{agent}/memory/projects/{name}.md
    try:
        for project in memory_updates.get("projects", []):
            if not isinstance(project, dict):
                logger.warning("Skipping non-dict projects item: %r", project)
                continue
        ...
        logger.exception("Error persisting projects for agent=%s; skipping field", agent_name)
    ...
    try:
        ...
        logger.exception("Error persisting agent_memory for agent=%s; skipping field", agent_name)
    ...
    try:
        ...
        logger.exception("Error persisting daily_log for agent=%s; skipping field", agent_name)
