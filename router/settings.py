...
_REGISTRY_ENTRIES: tuple[Setting, ...] = (
    ...
    Setting(
        ...
        "the stored MERGE_QUEUE_TRANSPORT/MERGE_QUEUE_CONVERSATION_REF settings. Mirrors "
        "DISPATCH_FEED_VIA_CHAT_ADAPTER (#713). Default-on since #859, which also deleted the "
        "raw-Slack (slack_post) fallback this flag used to guard — off now just skips the post "
        "(no transport/conversation_ref configured, or the flag explicitly disabled) rather than "
        "falling back to Slack.",
        reload="hot",
        category="Features",
    ),
    Setting(
